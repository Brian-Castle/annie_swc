...
annie-swc is for neuroscientists (and others) who need a reliable way to convert legacy SWC files into newer formats.
...
annie-swc will take an imperfect SWC tracing, and convert it into a set of watertight meshes. The set includes
...
to be saved, and annie-swc is an example. The goal is an end-to-end workflow starting with tracings, 
...
At the moment, annie-swc is more than a convenience, it addresses a gap in the integrity of the early stages 
of the open source workflow. There is no single tool that accomplishes the first part of the workflow on its
own. The problems range from seg faults to undocumented features. annie-swc curates the parts of each
...
the radius in the SWC file was too big. annie-swc will flag and identify these imperfections.
...
During the optimization phase you'll see a lot of output from VTK on the screen, just ignore it.
...
Documentation and a description of the Annie software suite is here: https://annie-interface.org
